...
class TestPixKeyInfoPatch(TestCase):
    ...
    def test_success_cancel(self):
        pix_keys = starkinfra.pixkey.query(status="created", limit=1)
        for pix_key in pix_keys:
            self.assertIsNotNone(pix_key.id)
            self.assertEqual(pix_key.status, "created")
            updated_pix_key = starkinfra.pixkey.update(
                id=pix_key.id,
                reason="userRequested",
                account_created="2022-01-01T12:09:09:00",
                account_number=f"\"{randint(10000, 99999)}\"",
                account_type=choice(["checking", "savings", "salary", "payment"]),
                branch_code=f"\"{randint(1000, 9999)}\"",
                name=get_full_name(),
            )
            self.assertEqual(updated_pix_key.reason, "userRequested")
            self.assertEqual(updated_pix_key.account_created, "2022-01-01T12:09:09:00")
