...
class TestPixClaimPage(TestCase):
    ...
    def test_success_with_params(self):
        pix_claims = starkinfra.pixclaim.page(
            limit=10,
            after=date.today() - timedelta(days=100),
            before=date.today(),
            status="failed",
        ...
        self.assertEqual(len(list(pix_claims)), 2)
...
class TestPixClaimInfoPatch(TestCase):
    ...
    def test_success_cancel(self):
        ...
        self.assertIsNotNone(pix_claim.id)
        self.assertEqual(pix_claim.status, "created")
        updated_pix_claim = starkinfra.pixclaim.update(
            id=pix_claim.id,
            reason="userRequested",
            status="canceled"
        )
        self.assertEqual(updated_pix_claim.status, "canceled")
